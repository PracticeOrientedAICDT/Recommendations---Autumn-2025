...
class DiffusionSlateModel(nn.Module):
    def __init__(self, dim=1024, hidden=320, layers=3, heads=10, dropout=0.2, slate_len=10,
                 ctx_layers=3, ctx_heads=8):
        super().__init__()
        ...
        self.ctx_encoder = nn.TransformerEncoder(ctx_encoder_layer, num_layers=ctx_layers)
...
def generate(model, prompt_emb, item_catalog_embs, diff_vars, args, device, num_inference_steps=50):
    ...
    loop = tqdm(range(num_inference_steps), desc="DDIM Sampling", total=num_inference_steps, leave=False)
    for i in loop:
        # Get current and previous timesteps
        t = timesteps[i].expand(B)
        # t_prev is the next step in the schedule, or -1 for the last step
        t_prev = timesteps[i+1] if i < num_inference_steps - 1 else torch.tensor(-1, device=device)
        ...
        if t_prev_val >= 0:
            _a_prev = a_t_all[t_prev].view(B, 1, 1) if isinstance(t_prev, torch.Tensor) else a_t_all[t_prev]
        else:
        ...
        # 3. DDIM update rule (with eta=0 for deterministic sampling)
...
def validate(model, val_loader, diff_vars, args, device, item_catalog_embs=None):
    ...
    for i, (x0, c) in enumerate(loop):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=(device.type=='cuda')):
            ...
            x0_cosine = F.cosine_similarity(pred_x0_norm, x0_norm, dim=-1).mean()  # (B, K) -> (B,) -> scalar
            running_x0_cosine += x0_cosine.item()
            
            # If we have catalog, check if we can retrieve the correct items
            if item_catalog_embs is not None and i == 0:  # Only check first batch to save time
                ...
                _, top_10_indices = torch.topk(sims, k=min(10, item_catalog_embs.size(0)), dim=-1)
    ...
    avg_total_loss = avg_loss_mse + (avg_loss_cos * args.loss_cosine_weight) + 0.1 * avg_loss_l1
...
def get_scheduler(optimizer, args, total_steps):
    def lr_lambda(current_step):
        ...
        progress = float(current_step - args.lr_warmup_steps) / float(max(1, total_steps - args.lr_warmup_steps))
...
def train(args):
    ...
    print(f"Sample prompt norm: {prompt_norm:.4f} (expected ~{math.sqrt(args.item_dim):.4f} for normalized)")
            ...
            for lst in lists:
                for movie in lst.get("movies", []):
                    ...
                    if film_id and film_id not in movie_titles:
                        movie_titles[str(film_id)] = movie.get("title", f"Unknown (ID: {film_id})")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (x0, c) in enumerate(loop):
            ...
            with torch.amp.autocast(device_type=device.type, enabled=(device.type=='cuda')):
                ...
                # 3. Semantically aligned with the prompt (the prompt->item mapping)
                ...
                if item_catalog_embs is not None and args.catalog_loss_weight > 0:
                    ...
                    item_similarity = F.cosine_similarity(pred_x0_norm, x0_norm, dim=-1)  # (B, K)
                    ...
                    c_expanded = c.unsqueeze(1).expand(-1, args.slate_len, -1)  # (B, K, D)
                    prompt_item_similarity = F.cosine_similarity(pred_x0_norm, c_expanded, dim=-1)  # (B, K)
                    ...
                    loss_catalog_retrieval = loss_catalog_retrieval + 0.3 * (1.0 - prompt_item_similarity).mean()
                
                # Combined loss with reconstruction and catalog components
                loss = loss_mse + args.loss_cosine_weight * loss_cos + 0.1 * loss_l1 + \
                       0.5 * loss_recon_cos + 0.3 * loss_recon_mse + \
        ...
        wandb.log({
            ...
            "gpu_mem_MB": torch.cuda.memory_allocated() / 1e6 if torch.cuda.is_available() else 0
        }, step=epoch)
        
        print(f"Epoch {epoch}/{args.epochs}  TrainLoss: {avg_train_loss:.6f}  ValLoss: {avg_val_loss:.6f}")
        ...
        if epoch % 5 == 0 or epoch == 1:  # Show every 5 epochs or on first epoch
            ...
            with torch.no_grad():
                ...
                if prompt_texts:
                    ...
                    actual_idx = val_dataset.indices[val_idx] if hasattr(val_dataset, 'indices') else val_idx
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"💾 New best model saved (val_loss {best_val_loss:.6f}) to {ckpt_path}")
    ...
    generated_slate_indices = generate(
        ...
        args, 
        device,
        num_inference_steps=50 # Use 50 steps as per paper
    )
...
def get_args():
    ...
    # Model
    parser.add_argument('--item_dim', type=int, default=384, 
    ...
    parser.add_argument('--layers', type=int, default=6, 
                        help="Paper's layer count")
    parser.add_argument('--heads', type=int, default=10, 
                        help="Paper's head count")
    parser.add_argument('--ctx_layers', type=int, default=3, 
                        help="Paper's context encoder layer count")
    parser.add_argument('--ctx_heads', type=int, default=8, 
                        help="Paper's context encoder head count")
    ...
    parser.add_argument('--nn_temperature', type=float, default=1.0,
