...
def generate(
    ...
    for i in tqdm(range(num_inference_steps), desc="Generating"):
        ...
        pred_noise = pred_noise_uncond + args.cfg_scale * (
...
def main():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="train_data_movielens.pt",
    ...
    parser.add_argument(
        "--item_dim", type=int, default=384, help="Item embedding dimension"
    )
    ...
    parser.add_argument(
        "--ctx_layers", type=int, default=3, help="Context encoder layers"
    )
    parser.add_argument(
        "--ctx_heads", type=int, default=8, help="Context encoder heads"
    )

    # Generation
    parser.add_argument(
        "--num_inference_steps", type=int, default=50, help="DDIM steps"
    )
    ...
    for i, idx in enumerate(generated_indices[0], 1):
        if catalog_index_to_film_id and idx < len(catalog_index_to_film_id):
            ...
            title = movie_titles.get(
                film_id_str, movie_titles.get(film_id, f"Unknown (ID: {film_id})")
            )
